...
# EU GDPR for AI processing · 1.3.1
...
## Article 22 by design since 1.3.0

The solely-automated limb no longer requires a declared conclusion. It is
satisfied either by the organisation's attestation, or by design: a
material or determinative decision purpose (applied from validated
proposed uses) combined with an engaging action that can run with no
technically enforced human gate (`composition.autonomous_external_send_possible`,
derived from captured connector actions). The restriction rule emits
`gdpr.article22_established`, and the paragraph 2 condition and safeguard
rules consume that emitted qualification. Whether a decision was actually
taken remains a separate, execution-level question.
