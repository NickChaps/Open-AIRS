...
Write-policy hardening and clearer wording, no logic changes.
`gdpr.article22_established` is flagged `engine_only`: the engine refuses
an assessment whose input facts already contain it, so the qualification
can only come from the restriction rule (by declaration through
`decision.solely_automated`, or by design). The restriction finding is
retitled "Article 22 exposure is established" and its summary now states
explicitly that whether an individual decision was actually taken is a
separate execution-level question.
