...
# EU AI Act core qualification and readiness · 1.3.1
...
## How the classification is derived since 1.3.0

The pack no longer needs a pre-filled legal conclusion. The chain runs on
observable facts:

1. the Annex III route is established from the mapped use cases, or from
   purpose-taxonomy tags for the employment points (4a and 4b);
2. the Article 6(3) rule screens the derogation and emits its outcome,
   true or false, as `aiact.article6_3_exception_established`;
3. the classification rule emits `aiact.high_risk_established` (and the
   route) when the route holds and the derogation demonstrably fails;
4. every provider and deployer obligation rule consumes the emitted
   conclusion.

An organisation can still attest `aiact.high_risk_confirmed` directly; a
bridge rule carries the attestation into the same derived fact. What no one
can do any more is have an extractor propose the conclusion: the attested
and emitted facts are flagged `derived` and stay out of the extraction
catalogue.
