...
AIR Framework does not arrive in an empty field. This page names the
neighbouring projects honestly, says what each one is in plain words, and
states how AIR relates to it. Facts below were checked against the linked
...
builds. It does not make AIR first at any of the individual pieces.
...
and relations. That graph shape is close to AIR's object graph, which makes
CALM a natural future import or export target. Note the naming coincidence:
...
declared licence. AIR cites it as prior art for the composition idea and
...
scores to EU AI Act expectations. It evaluates models; AIR qualifies
composed uses. Complementary by construction: COMPL-AI scores could enter
AIR as facts on a `model` object.
<https://compl-ai.org/>
...
code, around a runtime product. AIR sits on the other plane, qualification
and proof, and treats runtime platforms like this one as enforcement
evidence and future data sources.
...
organisation-wide; AIR is an open, versioned engine at the component and
use level that such suites could consume.
...
boundary between the two: AIR Framework exists precisely to derive the
...
dependencies. A natural import source for an AIR inventory.
...
authorisations from late 2026. Security-control oriented rather than
AI-law oriented, and a credible export format for AIR assessment records.
