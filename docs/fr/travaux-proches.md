...
AIR Framework n'arrive pas sur un terrain vide. Cette page nomme
honnêtement les projets voisins, dit ce que chacun est en termes simples,
et précise comment AIR s'y rapporte. Les faits ci-dessous ont été vérifiés
...
dépôt. Cela ne fait d'AIR le premier sur aucune des pièces prises
isolément.
...
est proche du graphe d'objets d'AIR, ce qui fait de CALM une cible
...
licence déclarée. AIR la cite comme antériorité de l'idée de composition
...
modèles ; AIR qualifie des usages composés. Complémentaires par
construction : les scores COMPL-AI peuvent entrer dans AIR comme faits sur
un objet `model`.
<https://compl-ai.org/>
...
documentaire et du code d'exemple, autour d'un produit de runtime. AIR se
place sur l'autre plan, la qualification et la preuve, et traite ce genre
de plateforme comme preuve d'enforcement et future source de données.
...
Elles sont fermées, larges et à l'échelle de l'organisation ; AIR est un
moteur ouvert et versionné au niveau du composant et de l'usage, que ces
...
C'est la frontière entre les deux : AIR Framework existe précisément pour
...
crédible pour les dossiers d'évaluation AIR.
